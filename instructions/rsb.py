...
# pattern RSB<c>{<q>} {<Rd>, }<Rn>, #0 with bitdiffs=[]
...
def aarch32_RSB_i_T1_A(core, regex_match, bitdiffs):
    ...
    # decode
    d = core.reg_num[Rd];  n = core.reg_num[Rn];  setflags = not (cond is not None);  imm32 = core.Zeros(32); # immediate = #0

    def aarch32_RSB_i_T1_A_exec():
        # execute
        if core.ConditionPassed(cond):
            (result, nzcv) = core.AddWithCarry(core.NOT(core.R[n]), imm32, '1');
            if d == 15:
...
def aarch32_RSB_r_T1_A(core, regex_match, bitdiffs):
    ...
    def aarch32_RSB_r_T1_A_exec():
        ...
        if core.ConditionPassed(cond):
            ...
            (result, nzcv) = core.AddWithCarry(core.NOT(core.R[n]), shifted, '1');
            if d == 15:
                          # Can only occur for A32 encoding
                if setflags:
                    core.ALUExceptionReturn(result);
...
patterns = {
    'RSB': [
        (re.compile(r'^RSB(?P<c>[ACEGHLMNPV][CEILQST])?(?:\.[NW])?\s(?:(?P<Rd>\w+),\s)?(?P<Rn>\w+),\s#0$', re.I), aarch32_RSB_i_T1_A, {}),
    ...
    'RSBS': [
        (re.compile(r'^RSBS(?:\.[NW])?\s(?:(?P<Rd>\w+),\s)?(?P<Rn>\w+),\s#0$', re.I), aarch32_RSB_i_T1_A, {}),
