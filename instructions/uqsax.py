...
log = logging.getLogger('Mnem.UQSAX')
...
def aarch32_UQSAX_T1_A(core, regex_match, bitdiffs):
    ...
    def aarch32_UQSAX_T1_A_exec():
        ...
        if core.ConditionPassed(cond):
            sum  = core.UInt(core.Field(core.R[n],15,0)) + core.UInt(core.Field(core.R[m],31,16));
            diff = core.UInt(core.Field(core.R[n],31,16)) - core.UInt(core.Field(core.R[m],15,0));
            core.R[d] = core.SetField(core.R[d],15,0,core.UnsignedSat(sum, 16));
            core.R[d] = core.SetField(core.R[d],31,16,core.UnsignedSat(diff, 16));
        else:
