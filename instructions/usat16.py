...
def aarch32_USAT16_T1_A(core, regex_match, bitdiffs):
    ...
    def aarch32_USAT16_T1_A_exec():
        ...
        if core.ConditionPassed(cond):
            (result1, sat1) = core.UnsignedSatQ(core.SInt(core.Field(core.R[n],15,0)), saturate_to);
            (result2, sat2) = core.UnsignedSatQ(core.SInt(core.Field(core.R[n],31,16)), saturate_to);
            core.R[d] = core.SetField(core.R[d],15,0,core.ZeroExtend(result1, 16));
            core.R[d] = core.SetField(core.R[d],31,16,core.ZeroExtend(result2, 16));
            if sat1 or sat2:
                core.APSR.Q = bool(1);
        else:
